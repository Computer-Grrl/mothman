...
#[derive(Debug, Clone)]
pub struct DevicePublicInfo{    // The device's ID
    device_id: [u8; 32],
    /// public key
...
impl UserId{
    ...
    fn create_info_package_with_pfp(&self, pfp: Option<ProfilePicture>) -> InfoPackageUploadRequest{
    ...
    fn sign(&self, data: &[u8]) -> Result<Ed25519Signature, Error> {
        self.keypair.try_sign(data).map_err(|e| Error::SigningError(e))
    }
